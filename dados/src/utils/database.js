...
// Desativa por padrão para qualquer número de dono
const defaultMsgBotOnEnabled = false; 

// Configuração limpa e com o SEU link oficial
ensureJsonFileExists(MSGBOTON_FILE, { 
  enabled: false, 
  message: `✨ *Gótica Bot Online!* ✨

Configurada por *Dev Soberano* 👑

💬 *Canal Oficial:*
https://chat.whatsapp.com/HhIATn48XsuAbduwn8sowT`
});
...
const getGroupRentalStatus = groupId => {
  ...
  // 🛡️ CORREÇÃO CRÍTICA: Proteção contra banco de dados ou objeto de grupos ausente
...
const setGroupRental = (groupId, durationDays) => {
  ...
  // 🛡️ CORREÇÃO CRÍTICA: Se o arquivo foi limpo, reinicia a estrutura padrão para evitar o erro de undefined
  if (!rentalData) {
    rentalData = { globalMode: false, groups: {} };
  }
  if (!rentalData.groups) {
    rentalData.groups = {};
  }
